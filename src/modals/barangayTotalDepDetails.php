<div class="add-modal fixed w-[500px] top-[10%] right-[25%] text-md ">
        <?php 
            ...
            if(isset($_POST['TdIDbtn'])){ 
                ...
                $totalDepDetails = mysqli_query($connectdb,"SELECT td.clTdID, td.clTdPercent, td.clTdYear, td.clBrID, td.clFtID, td.clItID, td.clUnID, cm.clCmID, AVG(cm.clCmPercent) AS aveChildMal,
                                                ...
                                                ON td.clBrID = $brgyID
                                                LEFT JOIN pmms.tbchildmalnutrition AS cm
                                                ON td.clBrID = cm.clBrID AND td.clTdYear = cm.ClCmYear
                                                LEFT JOIN pmms.tbfoodthreshold AS ft 
                                                ON td.clBrID = ft.clBrID AND td.clTdYear = ft.ClFtYear
                                                LEFT JOIN pmms.tbincomethreshold AS it 
                                                ON td.clBrID = it.clBrID AND td.clTdYear = it.ClItYear
                                                LEFT JOIN pmms.tbunemployment AS un
                                                ON td.clBrID = un.clBrID AND td.clTdYear = un.ClUnYear
            ...
            <?php 
                $row = $totalDepDetails->fetch_assoc();
                    ...
                    <div class="pb-8 pt-4 grid grid-cols-2">
                        ...
                        <p class="text-center text-lg font-semibold">'. round($row["aveChildMal"], 2).'</p>
                        <div class="col-span-2 text-[12px] mb-8">
