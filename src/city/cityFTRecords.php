...
<head>
    ...
    <title>City Food Threshold</title>
</head>
<body class="bg-[#FFF0B9] font-Poppins">
    ...
    <div class="flex">
        ...
        <div class="h-full ml-72 px-12 py-6 w-full">
            ...
            <h1 class="mt-4 text-2xl font-semibold tracking-wider text-orange-200">Food Threshold Records</h1>
                ...
                <table class="table-auto bg-white w-full text-[#623C04] text-left text-sm">
                    ...
                    <?php  
                        if(isset($_GET['search'])){ //checks if the input text is not null
                            ...
                            $FTdata = mysqli_query($connectdb,"SELECT ft.clFtID, ft.clBrID, ft.clFtYear, ft.clFtPercent, br.clBrID, br.clBrName 
                            FROM tbfoodthreshold as ft LEFT JOIN tbbarangay as br ON ft.clBrID = br.clBrID WHERE br.clBrName LIKE '%$filtervalues%' OR (ft.clFtYear = '$filtervalues')");
                        }
